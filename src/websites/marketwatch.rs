...
pub fn get_marketwatch_data(date: NaiveDate
) -> anyhow::Result<()> {
    let req = reqwest::blocking::get(MARKETWATCH)?;
    let document = scraper::Html::parse_document(&req.text()?);
    //let main_parent = &format!("div.element[data-tab-pane=\"{}/{}/{}\"]>", month, day, year);
    let main_parent = &format!("div.element[data-tab-pane=\"{}\"]>", date.format("%m/%d/%Y"));

    let css_symbol_selector = &format!(
        "{}{}",
        main_parent, SYMBOL_SELECTOR
    );
    let css_company_name_selector = &format!(
        "{}{}",
        main_parent, COMPANY_NAME_SELECTOR
    );
